...
- 대회 방식은 **전체 리그(Full League)** 또는 **스위스 리그(Swiss Tournament)** 두 가지를 지원합니다.
...
- AI 어시스턴트 패널 (Claude Opus / Gemini를 활용한 코드 도움)
...
| AI 어시스턴트 | Anthropic Claude API (`claude-opus-4-8`), Google Gemini API |
...
| 큐 | Redis (`code_battle_test_queue`, `code_battle_grading_queue`, `code_battle_swiss_league_queue`, `code_battle_full_league_queue`) |
| 지원 언어 | C++ / Java / Python |

### 인프라
| 분류 | 기술 |
|------|------|
| 클라우드 | AWS (EC2, S3, ECR, SSM, Auto Scaling Group) |
| 프론트 배포 | S3 Static Hosting + CloudFront |
| 백엔드 배포 | EC2 (JAR 직접 실행, SSM으로 배포 명령 전달) |
| 채점 서버 배포 | ECR Docker 이미지 → EC2 ASG (`asap-judge-server`) |
| DB | PostgreSQL 16 (AWS RDS 또는 EC2 Docker) |
...
├── front/                          # React 프론트엔드
│   ├── src/
│   │   ├── api/                    # Axios API 함수 (auth, contest, code, payment, SSE, AI)
...
- **Full League**: 모든 참가자가 서로 1:1 대결 (소규모 대회)
...
DB 스키마는 Flyway로 관리되며 마이그레이션 파일은 `server/src/main/resources/db/migration/` 에 위치합니다.
