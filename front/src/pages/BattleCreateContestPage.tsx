          ...
          <div className="cc-preview-panel">
            ...
            <div className="cc-preview-body">
              ...
              <div>
                ...
                {description.trim() ? <div className="cc-md-preview"><ReactMarkdown remarkPlugins={[remarkGfm]}>{description}</ReactMarkdown></div> : <div className="cc-md-empty">문제 설명이 없습니다.</div>}
              </div>
            </div>
          </div>
        </div>
                ...
                <section className="cc-section">
                  ...
                  <div className="cc-field">
                    <div className="cc-label-row">
                      ...
                      <button type="button" className="cc-import-btn" onClick={() => descImportRef.current?.click()}>📂 파일로 불러오기</button>
                      <input ref={descImportRef} type="file" accept=".md,.txt" style={{ display: "none" }} onChange={async (e) => { const f = e.target.files?.[0]; if (!f) return; setDescription(await f.text()); e.target.value = ""; }} />
                    </div>
